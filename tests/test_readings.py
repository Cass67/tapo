...
def test_normalizes_common_energy_fields():
    ...
    assert reading.name == "desk"
    assert reading.ip == "192.168.1.50"
    assert reading.power_w == 12.5
    assert reading.voltage_v == 240
    assert reading.current_a == 0.052
    assert reading.total_energy_kwh == 0.4
...
def test_normalizes_milli_and_wh_fields():
    ...
    assert reading.current_a == 0.052
    assert reading.total_energy_kwh == 0.4
    assert reading.raw["power_mw"] == 12500
