...
def test_cli_prints_discovery_guidance_when_no_devices(monkeypatch, tmp_path, capsys):
    ...
    assert result == 2
    assert "No Tapo devices are configured" in captured.err
    assert "Router DHCP leases" in captured.err
...
def test_cli_reports_missing_credentials(monkeypatch, tmp_path, capsys):
    ...
    config_path.write_text(json.dumps({"devices": [{"name": "desk", "ip": "192.168.1.50"}]}), encoding="utf-8")
    ...
    captured = capsys.readouterr()
    assert result == 2
    assert "TAPO_USERNAME" in captured.err
    assert "TAPO_PASSWORD" in captured.err
...
def test_cli_prints_table_and_writes_jsonl(monkeypatch, tmp_path, capsys):
    ...
    captured = capsys.readouterr()
    assert result == 0
    assert "desk\t192.168.1.50\t12.5" in captured.out
    assert json.loads(output_path.read_text(encoding="utf-8").splitlines()[0])["power_w"] == 12.5
...
def test_cli_suggests_discovery_when_all_devices_fail(monkeypatch, tmp_path, capsys):
    ...
    captured = capsys.readouterr()
    assert result == 1
    assert "ERROR desk" in captured.err
    assert "Run: tapo-probe --discover" in captured.err
...
def test_cli_reports_partial_success_summary(monkeypatch, tmp_path, capsys):
    ...
    def fake_collect(username, password, devices):
        return [normalize_reading("ok", "192.168.2.93", {"current_power": 22})], ["bad (192.168.2.58): HASH_MISMATCH"]
    ...
    captured = capsys.readouterr()
    assert result == 0
    assert "Summary: 1 succeeded, 1 failed" in captured.err
    assert output_path.exists()


def test_cli_discover_prints_found_devices(monkeypatch, capsys):
    monkeypatch.setattr(cli, "discover_devices", lambda timeout=5: [{"ip": "192.168.2.73", "model": "P110"}])

    result = cli.main(["--discover"])

    captured = capsys.readouterr()
    assert result == 0
    assert "192.168.2.73" in captured.out
    assert "P110" in captured.out
...
def test_cli_serve_starts_exporter(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("TAPO_PASSWORD", "secret")
    config_path = tmp_path / "config.json"
    config_path.write_text(json.dumps({"devices": [{"name": "desk", "hostname": "desk-host", "ip": "192.168.1.50"}]}), encoding="utf-8")
    called = {}
    ...
    assert result == 0
    assert called["config"].devices[0].hostname == "desk-host"
...
def test_cli_serve_uses_sys_argv_when_argv_is_none(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(cli, "serve_metrics", fake_serve)
    monkeypatch.setattr("sys.argv", ["tapo-probe", "serve", "--config", str(config_path), "--port", "9108"])
